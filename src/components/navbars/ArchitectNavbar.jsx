...
import { cn, logoutFireBase } from "@/utils";
import { Bars2Icon, ChatBubbleOvalLeftIcon } from "@heroicons/react/24/outline";
...
const ArchitectNavbar = (props) => {
  const cookies = useCookies();
  const id = Cookies.get("id");
  const { data: architect, isLoading } = useFetchData(
    `/archimatch_app/architect/find_architect_by_user/${id}`,
    "architect",
  );
  ...
  console.log(backend + architect?.data?.architect?.architect_avatar);

  return (
    <>
      <Navbar
        className={cn(
          pathname.includes("abonnement") || pathname.includes("Paiement")
            ? "lg:hidden"
          ...
          <div className="hidden lg:flex items-center gap-8 ">
            ...
            <div className="hidden lg:flex items-center gap-3 ">
              <div className="flex flex-col items-center">
                <div className="flex px-4 py-2 bg-[#F5F5F5] rounded-[50px]">
              ...
              <div className="w-[0.5px] h-[40px] bg-[#EAECF0]"></div>
              <Menu>
                <MenuHandler>
                  <Avatar
                    variant="circular"
