...
import { useCreateAnnoucement } from "@/services/queries";
...
const page = () => {
      ...
      formik: useFormik({
        ...
        onSubmit: () => {
          let api_data = validations.reduce((acc, validation, index) => {
            if (index === 11) {
              return { ...acc, user: validation.formik.values };
            }
            if (index === 5) {
  ...
  const steps = [
    ...
    {
      id: 11,
      component: <Step12 formik={validations[11].formik} />,
      image: CreateProject,
    },
  ];
  ...
  const CreateAnnoucementMutation = useCreateAnnoucement("", {});

  const handleSubmit = (values) => {
    CreateAnnoucementMutation.mutate({ ...values });
  };
